...
# Last edited by Elaina Passero 12/11/19 - Adapted for WR 674 Project
...
q.ep.weibull <- function(hydrograph,modeled_q){
  ...
  hydro_ep <- bind_rows(flow_scene_ep,data.frame(anti_join(df_m_q["discharge"],flow_scene_ep["discharge"]))) %>%
    arrange(discharge) %>%
    distinct() # removes duplicate entries (does not affect EP values)
  ...
  # Calculalte EP for flows smaller than flow scenario.
